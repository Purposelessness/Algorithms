...
class SolutionSequentialGrowth {
  struct Data {
    Graph graph;
    Path path;
  };
 ...
 public:
  ...
  [[nodiscard]] Path Solve(const Graph& graph, int estimated_len = INT_MAX) {
    starting_graph_ = graph;
    optimal_path_.length = estimated_len;
    Data data{graph, {}};
    print("Graph:\n");
    print_graph(graph);
    print("\n");
    ReduceMatrix(data.graph);
    BnB(std::move(data));
  ...
  [[nodiscard]] Path SolveChain(const Graph& graph, int starting_point,
    ...
    print_graph(graph);
    print("\n");
    print("Including edge: (" << exit_edge.y + 1 << ", " << exit_edge.x + 1
                              << ")\n\n");
    ReduceMatrix(data.graph);
    BnB(std::move(data));
    return optimal_path_;
  ...
  void IncludeEdge(Data data, const Point& edge) {
    ...
    data.path.length += starting_graph_.data[edge.y][edge.x];
#ifdef DEBUG
    std::cout << "Edges: ";
  ...
  static void ReduceMatrix(Graph& graph) {
    ReduceRows(graph);
    ReduceColumns(graph);
    print("Matrix reduced:\n");
    print_graph(graph);
  ...
  static void ReduceRows(Graph& graph) {
    for_column(0, _size, {
  ...
  static void ReduceColumns(Graph& graph) {
    for_row(0, _size, {
  ...
  [[nodiscard]] bool IsStillOptimal(const Data& data) const {
    ...
    if (!IsSimple(data.graph)) {
      ...
      auto tree_est = MinimalSpanningTreeEstimation(data.graph, data.path);
      if (tree_est == -1) {
        print("Cannot build spanning tree\n\n");
        return false;
      }
      if (tree_est > optimal_path_.length) {
  ...
  int TwoEdgesEstimation(const Graph& graph) const {
    int first = INT_MAX;
    int second = INT_MAX;
    foreach_column({
      foreach_row({
        if (is_valid(j, i)) {
          if (starting_graph_.data[j][i] < second) {
            second = _graph[j][i];
          }
          if (second < first) {
  ...
  int MinimalSpanningTreeEstimation(const Graph& graph,
    ...
    for (const Point& p : path.edges) {
      vertices.insert({p.x, p.y});
    }

    const auto& matrix = starting_graph_;
    auto comparer = [&matrix](const Point& l, const Point& r) {
      return matrix.data[l.y][l.x] > matrix.data[r.y][r.x];
    };
    ...
    while (vertices.size() < graph.data.size()) {
      ...
      length += matrix.data[edge.y][edge.x];
